...
void TestPanel::Init()
{
	Super::Init();
	{
		...
		this->SetRect(Shape::MakeCenterRect(WIN_SIZE_X, WIN_SIZE_Y, 1000, 1000));
		//this->SetRect(Shape::MakeCenterRectLTWH(0, 0, WIN_SIZE_X, WIN_SIZE_Y));
		{
			Image* image1 = new Image();
			image1->SetRect(Shape::MakeCenterRect(20, 90, 500, 500));//panel기준으로 해야하기 때문에 좌표가 같아야 함.
			//image1->SetRect(Shape::MakeCenterRectLTWH(50 + this->GetRect().pos.x, 50 + this->GetRect().pos.y, WIN_SIZE_X, WIN_SIZE_Y));//panel기준으로 해야하기 때문에 좌표가 같아야 함.
